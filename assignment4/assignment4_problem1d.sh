...
#SBATCH --job-name=assignment4_p1d
#SBATCH --time=00:30:00

set -euo pipefail

SCRIPT_DIR="${ASSIGNMENT4_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
SELF_PATH="${SCRIPT_DIR}/assignment4_problem1d.sh"
SCRIPT_PATH="${SCRIPT_DIR}/assignment4_problem1c.py"
RESULT_DIR="${RESULT_DIR:-${SCRIPT_DIR}/assignment4_problem1d_results}"
...
collect_results() {
  local summary_file="${RESULT_DIR}/problem1d_summary.txt"
  ...
  for workers in "${WORKERS[@]}"; do
...

    {
      ...
      if [[ -f "$result_file" ]]; then
        grep -E '^(workers=|cpus_per_task=|dataset=|max followers:|followers on average:|number of user with no followers:|num workers:|total time:)' "$result_file" || true
...
submit_jobs() {
  ...
  for workers in "${WORKERS[@]}"; do
    ...
    job_ids+=("$job_id")
    echo "submitted problem1d workers=${workers}, cpus=${workers}, job_id=${job_id}"
  done
