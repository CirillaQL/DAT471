...
#SBATCH --job-name=assignment4_p1b
#SBATCH --time=00:30:00

set -euo pipefail

SCRIPT_DIR="${ASSIGNMENT4_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
SELF_PATH="${SCRIPT_DIR}/assignment4_problem1b.sh"
SCRIPT_PATH="${SCRIPT_DIR}/assignment4_problem1a.py"
RESULT_DIR="${RESULT_DIR:-${SCRIPT_DIR}/assignment4_problem1b_results}"
...
collect_results() {
  local summary_file="${RESULT_DIR}/problem1b_summary.txt"
  ...
  for workers in "${WORKERS[@]}"; do
...

    {
      ...
      if [[ -f "$result_file" ]]; then
        grep -E '^(workers=|cpus_per_task=|dataset=|max follows:|users follow on average:|number of user who follow no-one:|num workers:|total time:)' "$result_file" || true
...
submit_jobs() {
  ...
  for workers in "${WORKERS[@]}"; do
    ...
    job_ids+=("$job_id")
    echo "submitted problem1b workers=${workers}, cpus=${workers}, job_id=${job_id}"
  done
